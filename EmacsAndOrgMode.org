...
| Software          | Platforms         | Notes                                        |
|-------------------+-------------------+----------------------------------------------|
| +[[http://aquamacs.org][Aquamacs]]+        | Mac               | No longer endorsed. Emacs 25/26 is outdated. |
|-------------------+-------------------+----------------------------------------------|
| [[https://bitbucket.org/mituharu/emacs-mac/src/master/][Mitsuharu's Emacs]] | Mac               |                                              |
|-------------------+-------------------+----------------------------------------------|
| [[https://www.gnu.org/savannah-checkouts/gnu/emacs/emacs.html][Gnu Emacs]]         | Windows/Mac/Linux | Easy install on Windows, Mac, and MacPorts   |
|-------------------+-------------------+----------------------------------------------|
...
As an update in November 2024, Aquamacs is outdated. Emacs is up to
version 29, but Aquamacs is based on Emacs 25.3. Thus, many newer
capabilities of Emacs are inaccessible on Aquamacs.
...
Short for *comp*lete *any*thing, [[http://company-mode.github.io][company-mode]] provides TAB completions
in Emacs buffers. It may be installed using a standard package manager
from the ELPA or MELPA repositories.

** =gptel=
[[https://github.com/karthink/gptel][gptel]] is an Emacs client for many large-language-model (LLM) AI tools.

** =org-roam=

[[https://www.orgroam.com/manual.html#Introduction][org-roam]] provides Wiki-style content development of linked Emacs
org files. These files represent linked nodes in a graph of information.

This capability is similar to the capabilities of [[https://obsidian.md][Obsidian]].
...
- written to the file system using =org-babel-tangle=. The Org-mode
  ...
  C-v t= (tangle a single code block) or =C-c C-v t= (tangle all code
  blocks).
  
** Some Code-block Macros
...
| Tangle single code block       | =C-u C-c C-v t= |
| Edit code block in buffer      | =C-c ,=         |
| Save code block, update buffer | =C-x C-s=       |
| Exit code block buffer         | =C-c ,=         |
|--------------------------------+-----------------|
...
*** Using Shell Scripts in Python

This is a powerful technique for using Python to run external programs. This is
particularly useful when you want to use third-party (i.e., non-Python) software
to manipulate files or data. One nice reference for this concept is in a
stackoverflow [[https://stackoverflow.com/a/29269316][thread]]. I have used this technique numerous times.

The technique here is:
1. Define a script stored in a Python variable
2. Then, define a ~subprocess.Popen~ object with the desired interpreter for
   your script.
   1. On Mac/Linux, you could use ~/bin/bash~ or the newer ~/bin/zsh~, or even specify
      a Python executable
   2. On a PC, you can specify ~cmd.exe~ or Python
3. Pass your script variable to your ~Popen~ object using the
   ~Popen.communicate()~ method
4. (Optional) Display results/output as desired

**** Mac/Linux-only Example

In this Mac-only example, we use the ~pwd~ command and the ~echo~ command. We
could just as easily invoke a more complex program, such as ~MATLAB~ or a
compiler, ~latex~, ~lyx~, ~pandoc~, ~quantumESPRESSO~, or any other program you
could imagine.
...
#+RESULTS:
: Script output:
: /Users/enrique_blair/Library/CloudStorage/GoogleDrive-epblair@gmail.com/My Drive/learning/computer_programming/EmacsAndOrgMode
: hello world
: 

**** Mac/Windows/PC

In this example, we can use the same concepts, but make the Python flexible
enough to run on Windows, as well. Here, we use ~os.name~ or ~sys.platform~ to detect the
operating system, and based on the value, we specify the appropriate syntax.

| OS      | ~os.name~ | ~sys.platform~ | ~platform.system()~ |
|---------+-----------+----------------+---------------------|
| Mac     | ~posix~   | ~darwin~       | ~Darwin~            |
|---------+-----------+----------------+---------------------|
| Windows | ??        | ??             | ~Windows~           |
...
#+begin_src python :results output
  ...
  if platform.system() == 'Darwin':
      # The script need not have indentation consistent
      # with the 
      script="""
  pwd # display the current working directory
  echo 'hello world'
  """
      ...
      """I'm not 100% sure the Windows script works
  ...
  # The remainder of this script is common to all platforms
  out, err = p.communicate( script )
...
*** Conda Environments in Emacs
I wanted to learn to use different Conda environments in Emacs. The
best thing I came across so far is the =conda.el= package ([[https://github.com/necaris/conda.el][GitHub]]).

To Install, I used =M-x package-refresh-contents=, and then
=M-x package-install conda=.

When I tried to use =M-x conda-env-activate=, I got this error:
#+begin_quote
There doesn't appear to be a conda or mamba executable in your exec
path. A common cause of problems like this is GUI Emacs not having
environment variable set up like the shell. See
https://github.com/purcell/exec-path-from-shell for a robust solution
to this problem.
#+end_quote

**** Tried (didn't work)
***** exec-path-from-shell (library)
The [[https://github.com/purcell/exec-path-from-shell][exec-path-from-shell page]] has this promising quote:
#+begin_quote
...
****** Usage
The documentation recommends using this after the
=(package-initialize)= command:
#+begin_src elisp :eval never
(when (memq window-system '(mac ns x))
  (exec-path-from-shell-initialize))
#+end_src

After doing this, I could use =M-x conda-env-activate=.
...
#+RESULTS:
: /Users/enrique_blair/anaconda3/bin/python3

***** =conda-env-activate-for-buffer=
Perhaps of greater interest to me than =conda-env-activate= is
=conda-env-activate-for-buffer=.

After using =M-x conda-env-activate-for-buffer=, the following code
block still pointed to the base installation! Not good.
#+begin_src python :results output
import sys
print(sys.executable)
#+end_src
...
Perhaps it works for a session associated with a notebook?
...
This didn't seem to help. It may be because of the following warning,
which I received when executing a session:
#+begin_quote
...
***** Setting the =org-babel-python-command= variable
Normally, I would use the following to set the 
#+begin_src lisp
(setq org-babel-python-command "~/anaconda3/envs/qiskit0.45/bin/python3")
#+end_src
Maybe I can set =org-babel-python-command= locally within a
variable. I think something to try is:
#+begin_src text :eval never
 * Local variables
 # local variables:
 # eval: (auto-fill-mode 0)
 #  eval: (poly-org-mode 0)
 # org-babel-python-command: '(("html" . "") ("php" . ""))
 # end:
#+end_src

Note: there was a missing parenthesis, so I added it and fixed it
here.

**** True (worked)
***** BEST: Have a =load_local_vars= script

Having a script like this seemed to work for me:
#+name: load_local_vars
#+begin_src emacs-lisp :results output :session :exports both
(setq-local org-babel-python-command (expand-file-name "~/anaconda3/envs/qiskit0.45/bin/python3"))
#+end_src

#+RESULTS: load_local_vars

The key here is to use =setq-local= to set a variable locally for a
given buffer.

Then, a nice test is a code block like this:
#+begin_src python :results output
import sys
print(sys.executable)
#+end_src
...
#+RESULTS:
: /Users/enrique_blair/anaconda3/envs/qiskit0.45/bin/python3

If this points to the right directory for a given environment, then I
...
*Note to Mac users.* As of May 31, 2024, I was unable to install
=org-ref= in Aquamacs. This is because on =M-x package-install
org-ref=, I would see =Package 'emacs-26.1' not available.= Since
Aquamacs is based on Emacs 25.X, this forces me to migrate to
[[https://emacsformacosx.com][Emacs for Mac OS X]]. I just installed Emacs version 29.3. Some notes
about installing =org-ref=:
1. If you see =[no match]= when trying to install packages, try
   pointing Emacs to [[https://melpa.org/#/][MELPA]]
   1. Add code like this to your Emacs preferences file (=~/.emacs=; [[https://melpa.org/#/getting-started][source]])
   2. Restart Emacs
   3. Use =M-x package-refresh-contents=
      1. I couldn't find packagkes until after this step! I was even
         able to use =M-x package-install org-ref=
